...
for zzz=1:10
    ...
    for zz=1:100
        Amounts=linspace(0.1,10,10);
        QS_AMT_1 = Amounts(zzz)*10^(-1);
        QS_AMT_2 = 5*10^(-1);
        QS_AMT_3 = 5*10^(-1);
        D = 8*10^(-10);
        S1 = 0.08;
...
scatter(repmat(Amounts(i)-0.05,[1,100]),AllNcells1(i,:),sz,d,'filled');
hold on
scatter(repmat(Amounts(i),[1,100]),AllNcells2(i,:),sz,c,'filled');
scatter(repmat(Amounts(i)+0.05,[1,100]),AllNcells3(i,:),sz,b,'filled');
end

errorbar(Amounts-0.05,y1,err1,'--xr','LineWidth',1.2);
hold on
errorbar(Amounts,y2,err2,'--xb','LineWidth',1.2);
errorbar(Amounts+0.05,y3,err3,'--xg','LineWidth',1.2);
legend("Strain 1","Strain 2","Strain 3");
xlabel("Rate of secretion of QS molecules from strain 1 (nmol/min)");
